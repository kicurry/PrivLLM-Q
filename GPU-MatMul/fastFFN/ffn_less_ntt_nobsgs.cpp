...
int main(int argc, char *argv[])
{
    ...
    BSGSConfig custom_bsgs{ .bs = 16, .gs = 1 };
    ...
    if (!multi_thread)
    {
        ...
        for (size_t group_k_idx = 0; group_k_idx < num_tiled_weight_rows; group_k_idx++)
        {
            if (group_k_idx % 64 == 0)
            {
                cout << group_k_idx << "-th group:" << num_col_per_act_ctxt - 1 << " rotations, "
                     << num_tiled_weight_cols << " * (" << num_col_per_act_ctxt << " multiplications)" << endl;
            }

            // 1. Baby-step Pre-Rotation
            const auto &input_ctxt = encrypted_activation[group_k_idx];
            vector<UnifiedCiphertext> rotated_ctxts(num_col_per_act_ctxt, backend);

            nvtxPush("Rot", backend);
            for (size_t i = 0; i < num_col_per_act_ctxt; i++)
            {
                evaluator.rotate_rows(input_ctxt, i * activation_rows, galoisKeys, rotated_ctxts[i]);
                evaluator.transform_to_ntt_inplace(rotated_ctxts[i]);
            }
            nvtxPop("Rot", backend);
            if (group_k_idx == 0)
            {
                init.print_noise_budget(rotated_ctxts[1], "after Rot");
            }
            ...
            for (size_t tiled_col_idx = 0; tiled_col_idx < num_tiled_weight_cols; tiled_col_idx++)
            {
                ...
                {
#ifdef PRE_ENCODING
                    evaluator.multiply_plain_ntt(rotated_ctxts[0], weight_ptxt[0], result_ctxt);
#else
                    evaluator.multiply_plain_ntt(rotated_ctxts[0], random_pt, result_ctxt);
#endif
                }
                else
                {
...
#ifdef PRE_ENCODING
                    evaluator.multiply_plain_ntt(rotated_ctxts[0], weight_ptxt[0], temp_ctxt);
#else
                    evaluator.multiply_plain_ntt(rotated_ctxts[0], random_pt, temp_ctxt);
#endif
                    evaluator.add_inplace(result_ctxt, temp_ctxt);
                }

                for (size_t i = 1; i < num_col_per_act_ctxt; i++)
                {
                    UnifiedCiphertext temp_ctxt(backend);
#ifdef PRE_ENCODING
                    evaluator.multiply_plain_ntt(rotated_ctxts[i], weight_ptxt[i], temp_ctxt);
#else
                    evaluator.multiply_plain_ntt(rotated_ctxts[i], random_pt, temp_ctxt);
#endif
                    evaluator.add_inplace(result_ctxt, temp_ctxt);
                    if (group_k_idx == 0 && i == 1 && tiled_col_idx == 0)
                    {
                        init.print_noise_budget(temp_ctxt, "after PMult");
                    }
                }
            }
            nvtxPop("MAC", backend);
            init.print_noise_budget(result_ctxts[0], "after" + std::to_string(group_k_idx) + "-th MAC:");
        }
        nvtxPush("iNTT", backend);
        for (size_t tiled_col_idx = 0; tiled_col_idx < num_tiled_weight_cols; tiled_col_idx++)
        {
            evaluator.transform_from_ntt_inplace(result_ctxts[tiled_col_idx]);
        }
        nvtxPop("iNTT", backend);
        evaluator.sync();
        printNVTXStats();
    }
    else
