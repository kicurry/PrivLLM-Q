...
int main(int argc, char *argv[])
{
    ...
    vector<vector<UnifiedCiphertext>> giant_ctxts(
        num_tiled_weight_cols, vector<UnifiedCiphertext>(giant_step, backend));
    ...
    for (size_t group_k_idx = 0; group_k_idx < num_tiled_weight_rows; group_k_idx++)
    {
        ...
        if (group_k_idx == 0)
        {
            init.print_noise_budget(baby_ctxts[1], "after BS-Rot");
        }
        ...
        for (size_t tiled_col_idx = 0; tiled_col_idx < num_tiled_weight_cols; tiled_col_idx++)
        {
            ...
            {
                auto &giant_ctxt = giant_ctxts[tiled_col_idx][i];
...
#ifdef PRE_ENCODING
                    evaluator.multiply_plain_ntt(baby_ctxts[0], weight_ptxt[i * baby_step], temp_ctxt);
#else
                    evaluator.multiply_plain_ntt(baby_ctxts[0], random_pt, temp_ctxt);
#endif
...
#endif
                    evaluator.add_inplace(giant_ctxt, temp_ctxt);
                    if (i == 1 && tiled_col_idx == 0 && group_k_idx == 0)
                    {
    ...
    init.print_noise_budget(giant_ctxts[0][0], "after BS-MAC");
    for (size_t tiled_col_idx = 0; tiled_col_idx < num_tiled_weight_cols; tiled_col_idx++)
    {
        auto &result_ctxt = result_ctxts[tiled_col_idx];
        nvtxPush("GS-Rot", backend);
        evaluator.transform_from_ntt(giant_ctxts[tiled_col_idx][0], result_ctxt);
...
#ifdef USE_HE_GPU
    throw std::runtime_error("multi-stream");
#endif
