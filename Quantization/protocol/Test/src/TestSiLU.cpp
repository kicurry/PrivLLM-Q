...
// silu ground truth (使用double精度)
// 使用标准SiLU公式: SiLU(x) = x * sigmoid(x) = x / (1 + e^(-x))
Tensor<double> silu_gt(Tensor<double> &input){
...
void test_silu(){
  ...
  const int scale = 13;
  he = new HE::HEEvaluator(ioArr[0], party, 8192,bitwidth*2,Datatype::HOST,{60,40,40});
  he->GenerateNewKey();
  he->print_parameters();
  if (party == ALICE){
    for (size_t i = 0; i < n; ++i){
      double v = -6+12 * static_cast<double>(i) / static_cast<double>(n - 1); // [-6,6]均匀分布
      // double v = 4;
      input_real(i) = v;
    ...
    gt = silu_gt(input_real);
  }
  
  SiLU<T> silu(fixpoint, he, bitwidth, scale);
  silu(input);  // in-place share of HE SiLU output
  ...
  if (party == ALICE){
    ...
    std::cout << "[SiLU] top mismatches (idx, in, gt, he, err):" << std::endl;
    ...
    std::cout << "[SiLU] sample recon[0]=" << recon(0) << std::endl;
    std::cout << "[SiLU] mae=" << mae << ", max_err=" << max_err << std::endl;
    std::ofstream ofs("silu_eval.csv");
    ofs << "input,gt,he\n";
    for (size_t i = 0; i < n; ++i){
      ofs << input_real(i) << "," << gt(i) << "," << recon(i) << "\n";
    }
    ofs.close();
    std::cout << "[SiLU] saved inputs/gt/he outputs to silu_eval.csv" << std::endl;
  } else {
